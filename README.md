...
[![GitHub stars](https://img.shields.io/github/stars/jvondev/acid-test?style=social)](https://github.com/jvondev/acid-test)
...
<img src="assets/hero-splash.png" alt="acid-test initializing gate and rain matrix engine" width="100%" />
...
`acid-test` is a specialized, zero-overhead adversarial audit engine. It injects deterministic chaos and fuzzed payloads directly into local or staging services to empirically verify that system state remains **Atomic**, **Consistent**, **Isolated**, and **Durable**.
...
npx @acid-test/cli audit --url http://localhost:3000
...
acid-test billing --url http://localhost:3000/api/webhooks/stripe --secret whsec_test

# Audit PostgreSQL Row Level Security (RLS) policies
acid-test db --db "postgresql://postgres:postgres@localhost:5432/app_db"

# Audit background workers against poison pills and crash recovery
acid-test queue --redis "redis://localhost:6379"
...
`acid-test` provides dedicated testing engines tailored to each distributed infrastructure domain:
...
<img src="assets/terminal-cockpit.png" alt="acid-test interactive terminal cockpit and code diff inspector" width="100%" />
...
`acid-test` delivers insights through two complementary interfaces:
...
<details>
<summary><b>Sample <code>acid-test.config.json</code></b></summary>

Create an optional `acid-test.config.json` in your repository root to configure targets and thresholds:
