...
export function registerDomainCommands(program: Command): void {
  const registerBilling = (name: string) =>
    program.command(name).description('Audit billing workflows, webhooks, and ledger idempotency').action((opts: any) => runBillingCommand({ ...program.opts(), ...opts }));
  ...
  const registerDb = (name: string) =>
    program.command(name).description('Audit RLS multi-tenant policies, SECURITY DEFINER, and query indexes').action((opts: any) => runDbCommand({ ...program.opts(), ...opts }));
  registerDb('db'); registerDb('pg'); registerDb('postgres'); registerDb('supabase'); registerDb('neon');

  const registerAuth = (name: string) =>
    program.command(name).description('Audit tenant separation, session replay, and JWT signature security').action((opts: any) => runAuthCommand({ ...program.opts(), ...opts }));
  ...
  const registerQueue = (name: string) =>
    program.command(name).description('Audit background workers, poison pills, and retry backoff').action((opts: any) => runQueueCommand({ ...program.opts(), ...opts }));
  registerQueue('queue'); registerQueue('bullmq'); registerQueue('sqs');

  const registerWebhook = (name: string) =>
    program.command(name).description('Audit raw-buffer HMAC, timestamp tolerance, and timingSafeEqual').action((opts: any) => runWebhookCommand({ ...program.opts(), ...opts }));
  ...
  const registerAi = (name: string) =>
    program.command(name).description('Audit streaming SSE abort signals, schema parsing, and 429 backpressure').action((opts: any) => runAiCommand({ ...program.opts(), ...opts }));
  registerAi('ai'); registerAi('openai'); registerAi('anthropic'); registerAi('gemini'); registerAi('ollama');

  const registerEmail = (name: string) =>
    program.command(name).description('Audit transactional templates, fallback parameters, and broken links').action((opts: any) => runEmailCommand({ ...program.opts(), ...opts }));
  registerEmail('email'); registerEmail('resend'); registerEmail('postmark'); registerEmail('sendgrid');

  const registerStorage = (name: string) =>
    program.command(name).description('Audit presigned URLs, MIME magic bytes, and public bucket exposure').action((opts: any) => runStorageCommand({ ...program.opts(), ...opts }));
